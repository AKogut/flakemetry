...
| **Information disclosure** | Reading another tenant's data or artifacts | Tenant scoping on all queries, **proven rather than asserted**: `packages/queries/src/__tests__/tenant-isolation.test.ts` seeds two structurally identical tenants and checks every project-scoped query for the other's ids, with a coverage guard that fails when a new query is added without one. Artifact keys are tenant-prefixed and served only through short-lived signed URLs. Auth cookies are `httpOnly` + `secure` (prod) + `sameSite`. |
...
| **Elevation of privilege** | A viewer changing settings; a read credential becoming a write one; a CI token silencing a failing test | Owner/admin role checks on token, policy, notification and quarantine management; tenant deletion is owner-only and needs the slug typed, checked server-side. The bulk export omits ingest token hashes, webhook signing secrets and the badge token, so a `read` credential cannot be turned into one that writes. Quarantine has its own token scope: hiding a failing test is neither reading nor reporting, and the credential pasted into every CI job must not carry it. |
...
- Tenant isolation is enforced in the query layer and tested there; Postgres row-level security is **not** enabled, so a query written without a tenant filter would be caught by the test suite rather than refused by the database.
- Load/soak testing and ingestion SLOs are tracked separately (#71).
